...
def main():
    ...
    optim_kwargs = {
        ...
        "prior_variance": 1,
        "acc_steps": 1,
        "warmup_kl_weighting_steps": 1000
    }
    ...
    for epoch in range(args.epochs):
        ...
        for data, target in train_loader:
            ...
            def closure2(surrogate_loss):
                optimizer2.zero_grad()
                output = model2(data)
                network_loss = F.binary_cross_entropy_with_logits(output, target)
                total_loss = network_loss - surrogate_loss
                total_loss.backward()
                return total_loss, output, network_loss

            loss1 = optimizer1.step(closure1)
            total_loss, _, loss2 = optimizer2.step(closure2)
